"""Step 15 终版配比与加权窗分配（plans/17 Task 1/2）。

**唯一权威定义在此**；dump 侧（``dump_c1_dataset.py`` 的配额公式）与训练侧
（``step14_offline.py --mix final15``）从这里取数，避免第三处重复表。
在线管线 ``step12_retrain.MIX_WEIGHTS`` 不再同步改动（在线路径无 imgvr 建池
代码，且 Step 14 起最终模型一律走离线；报告注明）。

终版有效权重（plans/17 §1.5 初值）：钉住四源 gtdb 0.20 / mrna 0.315 /
