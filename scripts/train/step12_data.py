"""Step 12 数据侧（plans/14_step12_partial_og2_retrain.md §1–2）：OG2 正式档多源池。

Step 11 的三源混合（NCBI/hg38/单池 OG2）升级为按 OG2 源分池加权：
...
- ``gtdb_v220_imgpr``：GTDB 细菌/古菌基因组 contig（记录带 accession）。
  ...
  （评估 + 复测 prompt 用），其余按 keep_prob 子抽样拼接进训练池。
...
完整性判据（2026-08-22 起，取代 Step 11 的"有 ``.aria2`` 伴侣即不存在"强校验）：
无伴侣即完整；有伴侣但 ``gzip -t`` 全流 CRC 通过视为**陈旧标记**、内容完备
（见 ``gz_intact``——双源 aria2 竞写时另一条进程可能攥着已收完文件的控制文件
不放）。注意本模块的宽容口径只覆盖 gtdb 路径（``iter_og2_jsonl_records`` /
``complete_train_chunks``）；其余 OG2 源走的 ``train.data.iter_og2_jsonl``
仍是 Step 11 的旧强校验，两套口径并存是有意的（那些源早已下完，无需放宽）。
与 plans/14 的两处执行时差异（盘点决定，报告须注明）：

1. gtdb fasta tar（batch1–3）均未下完 → 改用 ``gtdb_v220_imgpr`` JSON train
   chunk（用户指定只取前 2 个完整 chunk，本场景足够）；
2. ``eukaryotic_genic_windows``：Step 12 当时未完成故不参与；Step 13b/14
   起可选接入（``--with-euk``，权重 0.10），目标 bp 见 ``TARGET_BP``。

自检（只读盘点，不建池）::

    python scripts/train/step12_data.py --scan
"""
...
def build_gtdb_prompt_meta(meta: dict, ctx: int = 1024, window: int = 2048) -> list[dict]:
    """留出基因组 → ORF 密集编码区窗 prompt 描述（落盘供 step12_c5_retest 读取）。"""
    prompts: list[dict] = []
    for fam in sorted(meta.get("holdout_contigs", {})):
