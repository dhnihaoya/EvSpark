"""Step 15 CPU 测试：终版配比、加权窗分配器、epoch 计划、dump imgvr 接线。"""
...
from train.step15_mix import (  # noqa: E402
    FINAL_MIX,
...
def test_dump_imgvr_wiring():
    ...
    src = (_SCRIPTS / "dump_c1_dataset.py").read_text()
    assert '"imgvr": "imgvr_untagged"' in src
    # 配额公式为模块级纯 python，但模块 import train.* 链重；文本断言足够防回归
    assert "IMGVR_WEIGHT = 0.10" in src


def test_step14_offline_mix_flag_wired():
    src = (_SCRIPTS / "train" / "step14_offline.py").read_text()
