...
def _assert_states_close(snap_a, snap_b, what: str):
    from specdec.block.driver import diff_states
...
class _OracleGreedyDraft:
    ...
    def probs(self, prefix: np.ndarray) -> np.ndarray:
        from specdec.drafts import MASS_IDS, VOCAB_SIZE
...
def test_state_invariant_slice_mode(mini_model):
    ...
    from specdec.block.driver import get_seqlen_offset, snapshot_states
...
def test_slice_vs_snapshot_tokens_and_state(mini_model):
    ...
    from specdec.block.driver import snapshot_states
    from specdec.block.loop import spec_end_kv_len, speculative_generate
    from specdec.drafts import IdentityDraftModel
    from test_block_forward import PREFIX_LEN
