...
用法::
    ...
    CUDA_VISIBLE_DEVICES=0 python -u -m evspark.train.train_drafter \
      --max-positions 1000000 --results /tmp/train_drafter_smoke.json \
      --ckpt-name train_drafter_smoke.pt                # 1M 对拍冒烟
    CUDA_VISIBLE_DEVICES=0 python -u -m evspark.train.train_drafter \
      --scheme L27 --mix final15 --gamma 12 --max-positions 30000000 \
      --results benchmarks/train_drafter.json           # 全量
"""
...
from evspark.train.c1_pack import arrays_crc, dequantize_hidden
from evspark.train.distill import (
    EvalPack,
    assert_layers_exist,
    build_eval_packs,
    eval_drafter,
...
from evspark.train.data import sample_anchors
from evspark.train.corpora import (
    build_holdout_eval_records,
    gtdb_holdout_eval_records,
    load_gtdb,
    load_pool_pin,
    parse_holdout_pin,
)
from evspark.train.mix import FINAL_MIX, build_epoch_plan, load_mix
...
def resolve_scheme(args) -> tuple[str, tuple[str, ...]]:
    if args.scheme:
        ...
        sch = "L27"  # 定案默认：单层 blocks.27 注入（层选择不敏感，dump 最经济）
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--scheme", default=None,
                   help="注入层方案（默认 L27；若存在 benchmarks/step13b_layer_rescan.json 则读其判定）")
    ...
    p.add_argument("--mix", type=str, default="uniform",
                   help="源采样配比：uniform=盘面均匀（Step 14 口径）；final15=终版配比"
                        "（train.mix.FINAL_MIX）；或 JSON 字符串/文件路径")
    ...
    p.add_argument("--results", type=str, default="benchmarks/train_drafter.json")
